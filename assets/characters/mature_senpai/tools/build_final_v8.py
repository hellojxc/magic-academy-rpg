"""
Build mature_senpai_commercial_v8 from the Hunyuan/BlenderMCP v2 runtime asset.
...
- replace noisy arm/hand source geometry with more natural skinned anime arm and hand meshes
...
OUTPUT_GLB = PROJECT_ROOT / "public/assets/models/mature_senpai_commercial_v8.glb"
OUTPUT_PNG = PROJECT_ROOT / "public/assets/models/mature_senpai_commercial_v8.png"
OUTPUT_BLEND = PROJECT_ROOT / "assets/characters/mature_senpai/source/mature_senpai_commercial_v8.blend"
OUTPUT_REPORT = PROJECT_ROOT / "assets/characters/mature_senpai/mature_senpai_commercial_v8.report.json"
...
def make_parts(source: bpy.types.Object, armature: bpy.types.Object) -> tuple[list[bpy.types.Object], bpy.types.Collection]:
    collection = bpy.data.collections.new("MatureSenpai_CommercialV8_Parts")
    ...
    for part in PART_ORDER:
        ...
        if part == "Face_Head":
            add_face_morphs(obj)
        obj["commercial_v8_part"] = part
        obj["commercial_v8_retopo_layer"] = "source-preserving split runtime mesh"
        obj["commercial_v8_weight_profile"] = f"explicit profile weights for {part}"
        parts.append(obj)
...
def add_natural_arm_hand_meshes(
    ...
    skin = make_material("CommercialV8_NaturalAnimeSkin_ArmsHands", (0.94, 0.76, 0.68, 1.0), 0.58)
    nail = make_material("CommercialV8_SoftNails", (0.98, 0.86, 0.82, 1.0), 0.52)
    ...
    def build_side(side_name: str, sign: float) -> bpy.types.Object:
        ...
        def add_box(
            ...
            vertex_weights.extend([weights] * len(box_vertices))
            add_face([base, base + 1, base + 2, base + 3], material_index)
            add_face([base + 4, base + 7, base + 6, base + 5], material_index)
            add_face([base, base + 4, base + 5, base + 1], material_index)
            add_face([base + 1, base + 5, base + 6, base + 2], material_index)
            add_face([base + 2, base + 6, base + 7, base + 3], material_index)
            add_face([base + 3, base + 7, base + 4, base], material_index)
        ...
        for finger_offset, length, width in finger_specs:
            base_x = sign * (0.216 + finger_offset)
            center_z = 0.932 - length * 0.5
            add_box((base_x, -0.096, center_z), (width, 0.0032, length * 0.5), {hand: 1.0}, 0)
            add_box((base_x, -0.100, center_z - length * 0.42), (width * 0.82, 0.0012, 0.004), {hand: 1.0}, 1)

        thumb_x = sign * 0.240
        add_box((thumb_x, -0.090, 0.946), (0.004, 0.004, 0.020), {hand: 1.0}, 0)
        ...
        obj["commercial_v8_part"] = f"{side_name}NaturalArmHand"
        obj["commercial_v8_retopo_layer"] = "rebuilt clean arm/hand runtime mesh"
        obj["commercial_v8_weight_profile"] = f"explicit profile weights for {side_name}UpperArm/{side_name}LowerArm/{side_name}Hand"
        obj["commercial_v8_geometry_cleanup"] = "replaces noisy source Arms_Hands geometry with natural arm, palm, finger, and nail shapes"
        return obj
...
def add_face_detail_overlays(armature: bpy.types.Object, collection: bpy.types.Collection) -> list[bpy.types.Object]:
    ...
    overlay["commercial_v8_part"] = "Face_DialogueDetails"
    return [overlay]
...
def summarize_parts(parts: list[bpy.types.Object]) -> list[dict[str, object]]:
    ...
    for obj in parts:
        ...
        summaries.append({
            ...
            "retopoLayer": obj.get("commercial_v8_retopo_layer", "source-preserving split runtime mesh"),
            "weightProfile": obj.get("commercial_v8_weight_profile", "explicit profile weights"),
        })
    return summaries
...
def write_report(
    ...
    report = {
        "assetId": "mature-senpai-commercial-v8",
        "characterId": "mature_senpai",
        "status": "active-retopo-rigging-layer-preview",
        "source": "public/assets/models/mature_senpai_mcp_polish_v2.glb",
        "blendSource": "assets/characters/mature_senpai/source/mature_senpai_commercial_v8.blend",
        "output": "public/assets/models/mature_senpai_commercial_v8.glb",
        "thumbnail": "public/assets/models/mature_senpai_commercial_v8.png",
        "generatedBy": "BlenderMCP execute_code + assets/characters/mature_senpai/tools/build_final_v8.py",
        "runtime": {
            "defaultAsset": True,
            "format": "glb",
            "url": "/assets/models/mature_senpai_commercial_v8.glb",
            "thumbnailUrl": "/assets/models/mature_senpai_commercial_v8.png",
            "fallbackAssetId": "mature-senpai-commercial-v7",
        ...
        "runtimeDecision": {
            "defaultAsset": True,
            "fallbackAssetId": "mature-senpai-commercial-v7",
            "reason": "This v8 asset is the retopo/partition/morph/weight/secondary-bone delivery layer. It keeps the v7 likeness and arm/hand cleanup, adds explicit tip-level secondary bones for hair and skirt panels, stores per-part weight summaries, and exposes a machine-readable audit for the web runtime.",
        },
        "completedPasses": [
            "helper mesh removal",
            ...
            "separate palm, finger, thumb, and soft nail surfaces for dialogue-distance readability",
            "per-part skinning, morph, and secondary-bone audit report",
            "runtime GLB export with skins, morph targets, and animations",
        ],
        "limitations": [
            "The v8 retopo layer is a hybrid runtime retopo that preserves Hunyuan texture-critical triangles for likeness; it is not a fully artist-drawn quad topology over the whole body.",
            ...
            "The v8 arm/hand cleanup is more natural than v6 but still generated geometry; final art still needs hand-authored anatomy, knuckles, nails, and skin texture.",
