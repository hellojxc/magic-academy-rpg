"""
Build mature_senpai_commercial_v6 from the Hunyuan/BlenderMCP v2 runtime asset.
...
- replace noisy arm/hand source geometry with clean skinned anime arm meshes
...
OUTPUT_GLB = PROJECT_ROOT / "public/assets/models/mature_senpai_commercial_v6.glb"
OUTPUT_PNG = PROJECT_ROOT / "public/assets/models/mature_senpai_commercial_v6.png"
OUTPUT_BLEND = PROJECT_ROOT / "assets/characters/mature_senpai/source/mature_senpai_commercial_v6.blend"
OUTPUT_REPORT = PROJECT_ROOT / "assets/characters/mature_senpai/mature_senpai_commercial_v6.report.json"
...
def make_parts(source: bpy.types.Object, armature: bpy.types.Object) -> tuple[list[bpy.types.Object], bpy.types.Collection]:
    collection = bpy.data.collections.new("MatureSenpai_CommercialV6_Parts")
...
def add_clean_arm_hand_meshes(
    armature: bpy.types.Object,
    collection: bpy.types.Collection,
) -> list[bpy.types.Object]:
    skin = make_material("CommercialV6_CleanAnimeSkin_ArmsHands", (0.94, 0.76, 0.68, 1.0), 0.62)
    ...
    def build_side(side_name: str, sign: float) -> bpy.types.Object:
        ...
        rings = [
            add_ring((sign * 0.202, -0.054, 1.430), 0.020, 0.018, {"Chest": 0.28, upper: 0.72}),
            add_ring((sign * 0.232, -0.057, 1.315), 0.024, 0.020, {upper: 0.86, lower: 0.14}),
            add_ring((sign * 0.252, -0.059, 1.205), 0.023, 0.019, {upper: 0.24, lower: 0.76}),
            add_ring((sign * 0.235, -0.062, 1.090), 0.019, 0.017, {lower: 0.90, hand: 0.10}),
            add_ring((sign * 0.224, -0.070, 1.025), 0.020, 0.015, {lower: 0.22, hand: 0.78}),
            add_ring((sign * 0.220, -0.079, 0.982), 0.017, 0.012, {hand: 1.0}),
        ]
        for current, nxt in zip(rings, rings[1:]):
            bridge(current, nxt)

        top_center = len(vertices)
        vertices.append((sign * 0.200, -0.054, 1.448))
        vertex_weights.append({"Chest": 0.28, upper: 0.72})
        for segment in range(segments):
            faces.append([top_center, rings[0][segment], rings[0][(segment + 1) % segments]])

        palm_tip = len(vertices)
        vertices.append((sign * 0.220, -0.086, 0.955))
        vertex_weights.append({hand: 1.0})
        for segment in range(segments):
            faces.append([rings[-1][(segment + 1) % segments], rings[-1][segment], palm_tip])

        for finger_offset in (-0.018, -0.006, 0.006, 0.018):
            base_x = sign * (0.218 + finger_offset)
            base = add_ring((base_x, -0.088, 0.970), 0.0035, 0.005, {hand: 1.0})
            tip = len(vertices)
            vertices.append((base_x + sign * 0.002, -0.094, 0.940))
            vertex_weights.append({hand: 1.0})
            for segment in range(segments):
                faces.append([base[(segment + 1) % segments], base[segment], tip])

        mesh = bpy.data.meshes.new(f"MatureSenpai_{side_name}CleanArmHandMesh")
        mesh.from_pydata(vertices, [], faces)
        mesh.update(calc_edges=True)
        ...
        obj = bpy.data.objects.new(f"MatureSenpai_{side_name}CleanArmHand", mesh)
        collection.objects.link(obj)
        obj.parent = armature
        obj.matrix_parent_inverse = armature.matrix_world.inverted()
        armature_modifier = obj.modifiers.new("Production armature", "ARMATURE")
        ...
        obj["commercial_v6_part"] = f"{side_name}CleanArmHand"
        obj["commercial_v6_geometry_cleanup"] = "replaces noisy source Arms_Hands geometry"
        return obj
...
def add_face_detail_overlays(armature: bpy.types.Object, collection: bpy.types.Collection) -> list[bpy.types.Object]:
    ...
    overlay["commercial_v6_part"] = "Face_DialogueDetails"
    return [overlay]
...
def write_report(
    ...
    report = {
        "assetId": "mature-senpai-commercial-v6",
        ...
        "blendSource": "assets/characters/mature_senpai/source/mature_senpai_commercial_v6.blend",
        "output": "public/assets/models/mature_senpai_commercial_v6.glb",
        "thumbnail": "public/assets/models/mature_senpai_commercial_v6.png",
        "generatedBy": "BlenderMCP execute_code + assets/characters/mature_senpai/tools/build_final_v6.py",
        ...
        "runtimeDecision": {
            "defaultAsset": True,
            "fallbackAssetId": "mature-senpai-commercial-v5",
            "reason": "This v6 asset extends v5 with a targeted clean skinned arm/hand geometry replacement that removes the most visible dark source-mesh artifacts in dialogue-distance and full-body views while preserving the split skinned runtime mesh, six facial morph targets, face overlays, refreshed rule-painted weights, and secondary hair/skirt/strap/pendant bone hooks.",
        },
        "completedPasses": [
            "helper mesh removal",
            ...
            "targeted clean skinned anime arm/hand geometry replacement for noisy source Arms_Hands regions",
        ...
        "limitations": [
            ...
            "The arm/hand cleanup intentionally simplifies hand and finger detail to remove dark AI geometry artifacts; final art still needs hand-authored anatomy, nails, and skin texture.",
...
def main() -> dict:
    ...
    clean_arm_hands = add_clean_arm_hand_meshes(armature, collection)
