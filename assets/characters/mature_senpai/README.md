...
- `mature_senpai_mcp_retouch_v1.report.json`: active BlenderMCP retouch audit.
...
- `mature_senpai_mcp_retouch_v1.glb`: active Hunyuan-based BlenderMCP retouch with helper geometry removed, smoothed normals, material response polish, and `idle`, `walk`, and `talk` clips.
...
The BlenderMCP-retouched Hunyuan output is the active runtime preview because it
keeps the more lively AI-generated likeness. The procedural Blender template has
better separation and morph hooks, but it is too schematic to become the default
visual model.
...
The `mature_senpai_mcp_retouch_v1.glb` file is the current in-game animated
